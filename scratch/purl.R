...
D2 <- read_excel("data/STYCS_data_2023_260511.xlsx") |>
    ...
    dplyr::filter(!crop %in% c("Ley", "Fodder Beet", "Others", "Fallow")) |>
...
# ---------------------------------------------------------
D_Long_Agro <- D_Long |> filter(is.finite(z_inv_b_agro), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = droplevels(as.factor(crop)))
D_Long_Thm <- D_Long |> filter(is.finite(z_inv_b_agro), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = droplevels(as.factor(crop)))
D_Long_Cons <- D_Long |> filter(is.finite(z_inv_b_cons), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = droplevels(as.factor(crop)))
...
jobs <- list(
  mod_raw_co2 = function() {
    nlme(
        ...
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_CO2), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000), na.action = na.omit
    )
  },
  mod_thm_co2 = function() {
    nlme(
        ...
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site/year_f,
        weights = varPower(form = ~ fitted(.)),
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$a_CO2_total_mg_L), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000), na.action = na.omit
    )
  },
  mod_raw_aae = function() {
    nlme(
        ...
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site/year_f,
        weights = varPower(form = ~ fitted(.)),
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_AAE10), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000), na.action = na.omit
    )
  }
)
...
mod_cp_mechanistic <- nlme(
    ...
    data = D_CP_nona,
    fixed = list(C_base ~ crop, S_base ~ crop, beta_v0 + beta_invb ~ 1),
...
D_Yield <- D_ready |>
    ...
    filter(!is.na(z_ln_K), !is.na(z_pH), !is.na(z_ln_Mg), !is.na(z_fert_N), !is.na(z_Temp_Mean), !is.na(z_Prec_Anom)) |>
...
jobs_yield <- list(
  ...
  m_yield_raw_aae = function() {
    nlme(
        ...
        random = E_base ~ 1 | site,
        weights = varPower(form = ~ fitted(.)),
        start = c(10, rep(0, n_crops_yield - 1), 2, rep(0, n_crops_yield - 1), 0.05, rep(0, n_crops_yield - 1), rep(0, 7), 0),
        control = nlmeControl(maxIter = 2000, returnObject = TRUE)
    )
  }
)
...
D_res_all <- dplyr::bind_rows(
    ...
    D_Yield |> mutate(Model = "Raw $P_{CO2}$ (Conservative)", Fitted = predict(m_yield_cons_co2), Residual = residuals(m_yield_cons_co2)),
...
D_Pcrit_all <- dplyr::bind_rows(
    ...
    calc_pcrit(m_yield_cons_co2, "Raw P\\_CO2 (Conservative)"),
...
nlme_effects_all <- dplyr::bind_rows(
    ...
    extract_effects(m_yield_cons_co2, "Raw P\\_CO2 (Conservative)"),
