...
cat("\n--- Checking Correlation between z_inv_b_agro and z_v0 ---\n")
print(cor(D_Long_Agro$z_inv_b_agro, D_Long_Agro$z_v0))

n_crops <- length(levels(D_Long_Agro$crop))

cat("\n--- Testing mod_raw_co2 (start = 0, 0) ---\n")
mod_raw_co2 <- tryCatch({
    nlme(
...
cat("\n--- Testing mod_raw_co2 (start = 0.01, -0.01) ---\n")
mod_raw_co2_offset <- tryCatch({
    nlme(
        annual_P_uptake ~ ((V_max + beta_temp * z_Temp_Anom + beta_N * z_fert_N) * soil_0_20_P_CO2) /
            ((K_base * exp(beta_invb * z_inv_b_agro + beta_v0 * z_v0)) + soil_0_20_P_CO2),
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site/year_f,
        weights = varPower(form = ~ soil_0_20_P_CO2),
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_CO2), rep(0, n_crops - 1), 0.01, -0.01), control = nlmeControl(maxIter = 1000, opt = "nlm", pnlsTol = 0.05)
    )
}, error = function(e) paste("FAILED:", e$message))
print(mod_raw_co2_offset)
