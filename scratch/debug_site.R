...
job <- function() {
    nlme(
        ...
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site,
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_CO2), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000)
    )
}
tryCatch({
    res <- job()
    cat("\nSUCCESS with site only!\n")
}, error = function(e) {
    cat("\nERROR with site only:", e$message, "\n")
})
