...
cat("Checking where rows drop in the actual D_Yield pipeline...\n")
...
# Recreate what is in qi_code_subset5.R exactly
D_Y2 <- D_Y1 |> mutate(
...
cat("Step 2 (after agro mut):", nrow(D_Y2), sum(is.na(D_Y2$inv_b)), "\n")
...
cat("Step 3 (after cons mut):", nrow(D_Y3), sum(is.na(D_Y3$inv_b_cons)), "\n")
...
head(D_Y6 |> select(inv_b, inv_b_cons, n_pred_cons, ln_K_pred_cons, soil_0_20_P_CO2, b_power_cons))
