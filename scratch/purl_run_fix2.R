...
D2 <- read_excel("data/STYCS_data_2023_260511.xlsx") |>
    ...
    dplyr::filter(crop %in% c("WW", "WG", "SW", "KM", "SM", "KA", "ZR", "RA")) |>
...
# ---------------------------------------------------------
D_Long_Agro <- D_Long |> filter(is.finite(z_inv_b_agro), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = as.factor(crop)) |> droplevels()
D_Long_Thm <- D_Long |> filter(is.finite(z_inv_b_agro), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = as.factor(crop)) |> droplevels()
D_Long_Cons <- D_Long |> filter(is.finite(z_inv_b_cons), !is.na(z_fert_N), !is.na(z_Temp_Anom)) |> mutate(crop = as.factor(crop)) |> droplevels()
...
jobs <- list(
  ...
  mod_thm_co2 = function() {
    nlme(
        ...
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site/year_f,
        weights = varConstPower(form = ~ fitted(.)),
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$a_CO2_total_mg_L), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000)
    )
  },
  mod_raw_aae = function() {
    nlme(
        ...
        data = D_Long_Agro, fixed = list(V_max ~ crop, beta_temp ~ 1, beta_N ~ 1, K_base ~ crop, beta_invb ~ 1, beta_v0 ~ 1), random = V_max ~ 1 | site/year_f,
        weights = varConstPower(form = ~ fitted(.)),
        start = c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_AAE10), rep(0, n_crops - 1), 0, 0), control = nlmeControl(maxIter = 1000)
    )
  }
)
