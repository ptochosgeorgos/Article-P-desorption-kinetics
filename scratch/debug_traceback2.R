...
job <- function() {
    start_vec <- c(30, rep(0, n_crops - 1), 0, 0, median(D_Long_Agro$soil_0_20_P_CO2), rep(0, n_crops - 1), 0, 0)
    cat("Start vector NAs:", sum(is.na(start_vec)), "\n")
    ...
    nlme(
        ...
        start = start_vec, control = nlmeControl(maxIter = 1000)
    )
}
