...
format:
  ...
  pdf:
    ...
    include-in-header:
      text: |
        \usepackage{tikz}
        \usetikzlibrary{shapes, shapes.geometric, shapes.3d, arrows.meta, positioning, fit, backgrounds, calc}
    geometry:
      - top=25mm
      - left=25mm
      - right=25mm
...
# Helper function for dual-target plotmath rendering
tex_lab <- function(str) {
  if (knitr::is_latex_output()) {
    return(str) # TikZ parses raw TeX natively
  } else {
    return(latex2exp::TeX(str)) # svglite needs plotmath expressions
  }
}
```
