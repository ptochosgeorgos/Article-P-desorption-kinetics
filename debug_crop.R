...
modelling_data <- full_predictions %>%
  filter(
    year >= 1990, !is.na(soil_0_20_K_AAE10), soil_0_20_K_AAE10 > 0,
    !is.na(AR_K_final), AR_K_final > 0, !is.na(soil_0_20_Ca_AAE10), soil_0_20_Ca_AAE10 > 0,
...
print(nlevels(yield_data$crop))

# Let's trace model.matrix.default to see what x is
trace("model.matrix.default", tracer = quote({
  print("INSIDE model.matrix.default")
  print(object)
  print(nlevels(data$crop))
}), print=FALSE)
...
tryCatch({
  m_nlme_yield <- nlme(
    ...
    na.action = na.omit,
    control = nlmeControl(maxIter = 2, pnlsMaxIter = 1)
  )
}, error = function(e) {
  print(e)
})
