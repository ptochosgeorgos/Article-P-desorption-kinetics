...
curve_data <- bind_rows(
    ...
    make_curves("z_ln_K", "beta_K", "Soil K"),
    make_curves("z_ln_Mg", "beta_Mg", "Soil Mg"),
...
curve_data$Driver <- factor(curve_data$Driver, levels = c(
    "Physical Buffer Power (1/b)", "Soil pH", "Soil K", "Soil Mg", 
    "Nitrogen Fertilizer", "Mean Annual Temperature", "Precipitation Anomaly"
))

# Save fitted values for diagnostics chunk later
D_Yield$Fitted   <- predict(m_yield_nlme)
D_Yield$Residual <- residuals(m_yield_nlme)
...
nlme_effects <- broom.mixed::tidy(m_yield_nlme, effects = "fixed") |>
    filter(term != "c_base") |>
    mutate(
