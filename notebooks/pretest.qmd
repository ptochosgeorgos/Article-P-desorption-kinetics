...
p1 <- ggplot() +
  ...
  labs(x = "$Time (min)$",
       y = "$P_{V}(\\frac{mg}{L})$"); suppressWarnings(print(p1))
...
for (lvl in levels(d$ui)){
  ...
  nlme.coef[[lvl]]  <- cbind(coef(model4), ranefs, Rep=as.numeric(sub(".*_", "", rownames(coef(model4)))), ui=lvl, Site=d.tmp[1, "Site"], Treatment=d.tmp[1, "Treatment"], uid = rownames(coef(model4)))
...
soil_data_2022 <- allP %>%
  filter(!is.na(soil_0_20_P_CO2) & !is.na(soil_0_20_P_AAE10)) %>%
  group_by(uid) %>%
  slice_min(abs(year - 2022), n = 1, with_ties = FALSE) %>%
  ungroup() %>%
...
oxides <- read.csv(file = "data/oxides.csv", na.strings = c("", "n.a."))

D <- left_join(D,oxides,by="ui")

IEK <- read.csv(file = "data/IEK.csv", na.strings = c("", "n.a."))
