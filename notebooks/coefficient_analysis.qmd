...
D_thermo <- D |>
  mutate(
    # 1. Molarity for the 1:20 Kinetic Extraction
    K_mol_L  = (soil_0_20_K_H2O10 / 10) / (39.10 * 1000),
    Ca_mol_L = (soil_0_20_Ca_H2O10 / 10) / (40.08 * 1000),
    Mg_mol_L = (soil_0_20_Mg_H2O10 / 10) / (24.30 * 1000),
    
    # 2. Cationic Ionic Strength
    I_cations = 0.5 * ( (K_mol_L * 1^2) + (Ca_mol_L * 2^2) + (Mg_mol_L * 2^2) ),
    
    # 3. Calculate Total Positive Charge (Equivalents per Liter)
    Charge_Cations = (K_mol_L * 1) + (Ca_mol_L * 2) + (Mg_mol_L * 2),
    
    # 4. Assume matching negative charge from Monovalent Anions (z=1)
    # The I of monovalent ions is exactly 0.5 * concentration
    I_anions = 0.5 * (Charge_Cations * 1^2),
    
    # 5. Total Ionic Strength!
    Ionic_Strength = I_cations + I_anions,
    
    # 6. Davies Equation
    log_gamma = -0.509 * (1^2) * ((sqrt(Ionic_Strength) / (1 + sqrt(Ionic_Strength))) - (0.3 * Ionic_Strength)),
    gammma = 10^log_gamma,
    
    # 7. Apply to your already-fitted model parameters
    a_max = PS * gammma, 
    P_CO2_thermo = soil_0_20_P_CO2 * gammma
  )
```
...
p6 <- ggplot(D,aes(y=soil_0_20_P_AAE10, x=soil_0_20_P_CO2, col=Site, size = Treatment)) +
  ...
  labs(x="$P_{H_2O10}(mg/kg Soil)$",
         y="$P_{AAEDTA}(mg/kg Soil)$"); p6
  
p7 <- ggplot(D,aes(y=PS, x=soil_0_20_P_CO2, col=Site, size = Treatment)) +
  geom_point(shape = 7) +
  scale_x_log10() + scale_y_log10() +
  labs(x="$P_{CO_2}(mg/kg Soil)$",
          y="$PS(mg/kg Soil)$"); p7
...
fit.kin.Yrel     <- lmer(Ymain_rel         ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D)

fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k + log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment != "P166")
...
fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year), D, subset = Treatment != "P166")
r.squaredGLMM(fit.kin.Ynorm)


fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D)
r.squaredGLMM(fit.kin.Ynorm)

fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment == "P0")
r.squaredGLMM(fit.kin.Ynorm)
summary(fit.kin.Ynorm, ddf="Kenward-Roger")

fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment == "P100")
r.squaredGLMM(fit.kin.Ynorm)




fit.grud.CO2.Ynorm     <- lmer(Ymain_norm    ~ log(soil_0_20_P_CO2) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment != "P166")
r.squaredGLMM(fit.grud.CO2.Ynorm)
fit.grud.AAE10.Ynorm   <- lmer(Ymain_norm    ~ log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment != "P166")
r.squaredGLMM(fit.grud.AAE10.Ynorm)
fit.grud.CO2.AAE10.Ynorm <- lmer(Ymain_norm  ~ log(soil_0_20_P_CO2) * log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment != "P166")
...
fit.kin.Ynorm    <- lmer(Ymain_norm       ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D, subset = Treatment != "P166")
fit.kin.Pexport  <- lmer(annual_P_uptake  ~ k * log(PS) + scale(anavg_temp) + scale(soil_0_20_Corg) + (1|year) + (1|Site)  + (1|Site:block), D)
fit.kin.Pbalance <- lmer(annual_P_balance ~ k * log(PS) + scale(ansum_prec) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block), D)
...
fit.grud.CO2.Yrel     <- lmer(Ymain_rel       ~ log(soil_0_20_P_CO2) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)
fit.grud.AAE10.Yrel     <- lmer(Ymain_rel       ~ log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)
fit.grud.Yrel     <- lmer(Ymain_rel       ~ log(soil_0_20_P_CO2) * log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)


# this is hopeless, since cannot log becaus of 0's
fit.CO2.Pexport     <- lmer(annual_P_uptake     ~ log(soil_0_20_P_CO2) + scale(anavg_temp) + scale(soil_0_20_Corg) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)

fit.AAE10.Pexport     <- lmer(annual_P_uptake ~ log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_Corg) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)

fit.grud.Pexport     <- lmer(annual_P_uptake     ~ log(soil_0_20_P_CO2) * log(soil_0_20_P_AAE10) + scale(anavg_temp) + scale(soil_0_20_Corg) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)


fit.CO2.Pbalance     <- lmer(annual_P_balance      ~ log(soil_0_20_P_CO2) + scale(ansum_prec) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)


fit.AAE10.Pbalance     <- lmer(annual_P_balance      ~ log(soil_0_20_P_AAE10) + scale(ansum_prec) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)


fit.grud.Pbalance     <- lmer(annual_P_balance      ~ log(soil_0_20_P_CO2) * log(soil_0_20_P_AAE10) + scale(ansum_prec) + scale(soil_0_20_clay) + (1|year) + (1|Site)  + (1|Site:block) + (1|Site:Treatment), D)
...
ggplot(correlation_data, aes(x = E_mod_10080, y = PS)) +
  ...
  labs(
    title = "Relationship between Desorbable P and Exchangeable P",
    x = "Isotopically Exchangeable P, $E_{10080}$ (mg kg$^{-1}$)",
    y = "Desorbable P, $P_{desorb}$ (mg L$^{-1}$)"
  ) +
  theme_bw()
...
ggplot(correlation_data, aes(x = n_1440, y = k)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(
    title = "Relationship between Desorption Rate and Exchange Rate",
    x = "IEK Kinetic Parameter, $n_{60}$",
