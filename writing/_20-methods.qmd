...
Because the rate constant ($k$) is structurally invariant, it can be mathematically coupled with the historical Intensity data. To extrapolate this kinetic property to all treatments—including those where $k$ was not empirically measured (e.g., control plots and unmeasured sites like Grabs)—we developed a fundamental **Pedotransfer Function (PTF)** based on the Arrhenius equation. We modeled $\ln(k)$ utilizing the ratio of amorphous aluminum to iron oxides ($\ln(Al_{ox} / Fe_{ox})$) and soil pH. This ratio elegantly captures the competing activation energies of the dominant sorption surfaces, allowing us to accurately predict a rate constant ($k_{pred}$) for every plot in the dataset.

From these parameters, a novel predictor for agronomic availability, the **Dynamic Desorption Velocity ($v_0$)**, was defined mathematically as the product of the predicted rate constant and the historical static concentration:
