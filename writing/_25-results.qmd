...
D_long_agronomic <- D_long_agronomic %>%
  mutate(Variable = case_when(
    ...
    Variable == "Ymain_rel"        ~ "$Y_{rel}$ (\\%)",
    Variable == "annual_P_uptake"  ~ "$P_{up}$ (kg ha$^{-1}$)",
    Variable == "annual_P_balance" ~ "$P_{bal}$ (kg ha$^{-1}$)",
    TRUE                           ~ Variable
  )) %>%
...
D_long_soil <- D_long_soil %>%
  mutate(Variable = case_when(
    Variable == "k"                   ~ "$k$ (min$^{-1}$)",
    Variable == "PS"                  ~ "$P_{desorb}$ (mg L$^{-1}$)",
    Variable == "kPS"                 ~ "$J_0$ (mg L$^{-1}$ min$^{-1}$)",
    Variable == "soil_0_20_P_CO2"   ~ "$P_{CO_2}$ (mg kg$^{-1}$)",
...
knitr::kable(results_table_ynorm, escape = FALSE, row.names = FALSE, booktabs = TRUE)
...
knitr::kable(results_table_yrel, escape = FALSE, row.names = FALSE, booktabs = TRUE)
...
knitr::kable(results_table_export, escape = FALSE, row.names = FALSE, booktabs = TRUE)
